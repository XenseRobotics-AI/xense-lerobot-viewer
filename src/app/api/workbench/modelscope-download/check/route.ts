...
export async function GET(): Promise<Response> {
  try {
    return Response.json(
      { destinationRoot: defaultModelScopeDownloadRoot() },
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    const parsed = await parseModelScopeDownloadRequest(body);
    return Response.json(
      await runModelScopeDownloadCheck(parsed, request.signal),
      { headers: noStoreHeaders() },
    );
  } catch (error) {
