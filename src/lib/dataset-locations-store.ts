...
/**
 * One level of the filesystem for the folder picker: the subdirectories of
 * `value` (dot-directories hidden), each flagged when it is itself a dataset.
 * A directory that cannot be read yields an empty listing rather than an
 * error, so the picker can still step back out of it.
 */
export async function browseDirectory(value: string): Promise<BrowseListing> {
  const target = normalizeLocationInput(value);
  ...
  const listed = await Promise.all(
    directories.map(async (entry) => {
      ...
      return { name: entry.name, path: full, isDataset: await isDataset(full) };
    }),
  );
  return {
    path: target,
    parent: parent === target ? null : parent,
    isDataset: await isDataset(target),
    entries: listed,
  };
}
