...
/** The shape every healthy dataset has; anything else is flagged. */
...
/**
 * Flags a dataset whose state dimension or camera count departs from the norm.
 *
...
export function shapeAnomalyOf(
  ...
  const dimOdd = stateDim !== null && stateDim !== NORMAL_STATE_DIM;
  const streamsOdd = videoStreams !== NORMAL_VIDEO_STREAMS;
