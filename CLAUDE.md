...
- `DatasetCardGrid` takes `canDelete`; it is false when the browsed path is **outside** the root, hiding both the per-card Delete and the `TrashStrip`, because `local-dataset-trash.ts` refuses a dataset that is not inside the root. It is deliberately not `browsePath === root`: a location that happens to sit _under_ the root — `<root>/TacVerse-RDT-Test`, say — holds datasets the trash route accepts, so hiding Delete there withheld an action that would have worked. The test is `isPathInsideRoot` in `src/utils/browsePath.ts`, lexical because it decides what the UI offers; the route re-checks with `realpath`.
...
- Episode metadata can span **multiple chunks** (when episode count exceeds `chunks_size`). Always walk via the `iterateEpisodeMetadataFilesV3(repoId, version)` async generator in `fetch-data.ts` — it advances chunk-000 → chunk-001 → … Never hardcode `chunk-000`. The walk normally stops on a **count**: one metadata row is one episode, so it ends as soon as it has read `info.total_episodes` rows. The old terminator — request the next shard, take the 404 as the end — is still there underneath and still correct, but it fires only when `info.json` under-reports, so an ordinary dataset no longer logs two 404s per walk. The stop is safe because the episode list itself is `Array.from({length: info.total_episodes})`: an episode past that count is one the UI can never ask for.
...
- **`HF_ENDPOINT` defaults to `https://hf-mirror.com` for large downloads; catalog/statistics use `https://huggingface.co` by default** — the mirror is useful for payload bandwidth, but its metadata/file endpoint can redirect. Set `HF_CATALOG_ENDPOINT` only when a compatible metadata mirror is available.
